...
export default function File({ file, fromTrash }: FileProps) {
  ...
  const handleCloseNotify = (event: any, reason: string) => {
    if (reason === 'clickaway') {
      return
    }
    setOpenNotify(false)
  ...
  const action = (
    <React.Fragment>
      <Button
        color='secondary'
        size='small'
        onClick={(e) => handleCloseNotify(e, 'clickaway')}
      >
        UNDO
      </Button>
      <IconButton
    ...
    <>
      {file && (
        <div className={`file ${mode}`}>
          <div className='file-information' onClick={previewFileMain}>
